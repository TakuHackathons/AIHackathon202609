...
export const users = sqliteTable(
  ...
  (t) => [
    ...
    check('users_role_school', sql`(${t.role} = 'super_admin' AND ${t.schoolId} IS NULL) OR (${t.role} IN ('admin','general') AND ${t.schoolId} IS NOT NULL)`),
  ],
);
...
export const publicUser = (u: User) => ({ id: u.id, schoolId: u.schoolId, username: u.username, name: u.name, email: u.email, department: u.department, subjects: u.subjects, responsibilities: u.responsibilities, role: u.role, createdAt: u.createdAt });