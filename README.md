...
一般チャットは学校コードを指定せずに利用できます。この場合は学校固有のデータを参照せず、OrcaRouterによる一般的な対話を行います。

学校の時間割、施設、規則などを回答へ反映する場合は、チャット画面上部の「学校・学籍番号を設定」から学校コードを入力します。学籍番号は任意で、入力するとその学生の履修、課題、出欠を回答へ反映します。設定内容はブラウザのローカルストレージに保存され、同じ画面から変更・解除できます。
...
管理者用の登録用パスワードは自動生成され、24時間有効です。発行結果のコピーボタンからパスワードをコピーして対象の教員へ共有します。登録用パスワードからPasskeyを追加しても、すでに登録されているPasskeyは維持されます。
